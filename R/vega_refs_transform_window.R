...
#' @param groupby Field[] The data fields to by which to partition data objects into separate windows. If not specified, a single group containing all data objects will be used.
#' @param ops String[] The window or aggregation operations to apply within a window, including `rank`, `lead`, `sum`, `average` or `count`. See the [window operation reference](#ops) for more.
#' @param fields Field[] The data fields for which to compute aggregate or window functions. This array should align with the _ops_, _as_, and _params_ arrays. Field values can be `null` for operations that do not operate over a specific data field, including `count`, `rank`, and `dense_rank`.
#' @param params Array Parameter values for window functions. This array should align with the _ops_ array. Parameter values can be `null` for operations that do not accept a parameter (such as aggregation operations).
